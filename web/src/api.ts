...
export interface QuestionInfo {
  ...
  multiple?: boolean;
  /**
   * A free-text answer is accepted alongside the options. Absent means
   * accepted: OpenCode's schema defaults it to true, so only an explicit
   * `false` restricts a question to its options.
   */
  custom?: boolean;
}
