...
export function buildFixtureSessions(): MockSessionState[] {
    ...
    pendingQuestions: [
      {
        ...
        questions: [
          ...
          },
          {
            ...
            header: 'Branch',
            options: [
              { label: 'master', description: 'Straight to the deployed site' },
              { label: 'A new branch', description: 'Opens a pull request' }
            ],
            custom: false
          }
        ],
  ...
  const docs: MockSessionState = {
    ...bareState({
      id: 'ses_docs',
      provider: 'cloudflare',
      repoKey: 'acme/handbook',
      model: 'anthropic/claude-sonnet-4-5',
      title: 'Speed up CI with a warm pnpm store and a four-way test split',
      displayTitle: 'Speed up CI with a warm pnpm store and a four-way test split',
      phase: 'working',
      status: 'sleeping',
    ...
    transcript: shortTranscript({
      prompt: 'Cut the CI wall time. Caching and parallelism are both on the table.',
      reply:
        'Caching the pnpm store and splitting the suite four ways takes the pipeline from 11 minutes to 4. The change is all in `.github/workflows/ci.yml`.',
      baseMinutesAgo: 4 * 60
    }),
    mirroredAt: hoursAgo(4)
  };
