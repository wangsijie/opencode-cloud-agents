...
`pnpm run deploy` on every push to `master` or `main`. **Pushing to `master`
ships to production.** There is no separate release step and no manual
`pnpm run deploy` to offer afterwards.

The workflow deploys whichever account it has credentials for, and the gate is
the presence of `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID`, not the
repository name — that is what lets a fork run its own deployment, and it is
also what stops one from touching ours, since a fork inherits no secrets. Do not
reintroduce a `github.repository ==` condition: it would break every fork to
re-state something the secrets already guarantee.

Two consequences for anything added here. The workflow is the only supported
install path for an operator with no local toolchain, so a new Cloudflare
resource must be created *by the workflow* (idempotently — check, then create),
not just documented as a `wrangler` command to run by hand. And a new Worker
secret belongs in the "Sync the sandbox host's R2 secrets" pattern: written only
when absent, because `wrangler secret put` publishes a new version and evicts
the Durable Objects, which on every push would disturb live sessions for
nothing.
