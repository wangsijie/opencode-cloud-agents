...
A dedicated page at `/prebuilds` — the router grows a fourth route beside
`list`/`settings`/`session`, the sidebar a link beside Settings, `App.tsx` a
`PrebuildsPage` branch. Not a settings section: settings rows are key-value
edits, while this page has live, polling state.

The page is **one list, keyed by repo** — the same catalog the composer reads
(so it needs no new repo source and sorts in the same last-used order), merged
with `GET /api/prebuilds`. Every repo appears whether or not it has a
prebuild; there is no separate picker form, the row *is* the trigger. Per row:
...
- **Repo name**, and the prebuild state: `built 2 h ago · cloudflare · 1.4 GB`
  ...
  R2 object size / `du` on the volume generation), or `no prebuild` muted.
...
- **Build / Rebuild button.** Disabled with a spinner-label while that repo's
...
- **While a run is active**, the row expands into the step ladder — `clone →
  ...
  page's reason to exist: watching exactly where a prebuild's minutes go.
...
running mid-install, one failed — so the page is buildable without a backend.
...
`/prebuilds` page (mock fixtures first, then live), `bootStep` and
`workspaceOrigin`. Done means: trigger a run from the page, watch the step
